...
1. 双击 `start_gui.bat`（CMD 只会在启动瞬间闪过，不会与 GUI 一直同时显示）；
...
看到中文覆盖在原文字附近，就说明已经运行成功。工作台在实时翻译就绪后会隐藏，屏幕顶部会出现居中的只读状态条；需要调整或结束时，双击系统托盘中的 RefraTranslator 图标唤回工作台。工作台运行中点击右上角 **×** 会重新隐藏到托盘，翻译继续；点击顶部的 **停止翻译** 才会执行保存并停止流程。

以后使用时，只需双击 `start_gui.bat`，选择已经保存的游戏配置，再点击 **开始翻译**。每个配置会独立保存捕获区域、OCR、翻译、覆盖层和运行参数；切换游戏配置时会整套切换，不会沿用另一个游戏的设置。主题、窗口状态、已下载模型文件和设备检测结果仍由本机共享。需要让 CMD 保持可见、实时查看启动输出时，改用 `start_gui(debug).bat`；两个入口调用的是同一个启动后端。
