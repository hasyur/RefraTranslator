...
1. 双击 `start_gui.vbs`（启动时不会显示 CMD 窗口）；
...
以后使用时，只需双击 `start_gui.vbs`，选择已经保存的游戏配置，再点击 **开始翻译**。每个配置会独立保存捕获区域、OCR、翻译、覆盖层和运行参数；切换游戏配置时会整套切换，不会沿用另一个游戏的设置。主题、窗口状态、已下载模型文件和设备检测结果仍由本机共享。`start_gui.bat` 仍保留为排错入口，需要直接查看启动诊断时再使用。
