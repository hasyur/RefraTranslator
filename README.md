...
6. 在鼠标穿透的透明窗口中模糊原文字区域，可在“黑化模糊”和“仅模糊”间切换，并用带细描边的清晰字体绘制中文译文；
...
启动器的“译文背景”提供两种覆盖效果：“黑化模糊”在模糊区域叠加暗层以增强白色译文对比度；“仅模糊”根据文字框周边像素估算低频背景后再模糊，避免原文字的灰白或黑色笔画扩散成矩形色块。该估算不使用额外模型，并随文字背景缓存。保存运行设置后，新启动的实时翻译会使用所选模式。
...
启动器中的“实验性动态 ROI”提供另一条可随时关闭的实时调度路径，默认保持关闭。启用后，程序先执行一次整帧 OCR 建立画面与文字基线；随后扫描低分辨率变化热图，把连续变化合并到最新帧，并按配置的最小间隔执行 Paddle 局部 OCR。OCR 忙碌时不会累积旧帧，只有成功接受结果后才推进基线；局部扫描只更新紧变化种子关联到的旧文字，ROI 安全边距内的其他静态文字不会被误判为消失。变化过于广泛、ROI 过多或覆盖率过高时会安全回退为整帧 OCR。GUI 中两条路径共用的“变化检测频率”始终显示；开启动态 ROI 时隐藏旧全帧路径的“画面稳定补扫 / 静止画面兜底 / OCR 冷却”，改为显示“ROI 稳定等待 / ROI OCR 间隔 / ROI 最大合并”。切换路径不会重置被隐藏的数值。
